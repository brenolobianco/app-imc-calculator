...
const createQuery = ({
  oldPassword,
  newPassword,
}: UpdateOwnPasswordParams): string => `
mutation {
...
export default (): void => {
  ...
  const fakeBody = { oldPassword: '123456', newPassword: '654321' };
  utils.testIsGqlAuthenticated(app, resolver, createQuery(fakeBody));

  test('422 Password incorrect', () => {
    return baseRequest(fakeBody, setupData.dev.token).then(
      utils.expectGqlError(err.PASSWORD_INCORRECT, resolver),
    );
  });
  ...
  test('200 Password updated', () => {
    ...
    return baseRequest(body, setupData.dev.token).then(response => {
  ...
  test('422 Password incorrect - already updated', () => {
    ...
    return baseRequest(body, setupData.dev.token).then(
      utils.expectGqlError(err.PASSWORD_INCORRECT, resolver),
    );
  });
