...
const createEnts = async (): Promise<Ents> => {
  ...
  const o3 = await helpers.createAdmin({});
...
export default (): void => {
  ...
  test('422 Admin in use - by client', async () => {
    const admin = await helpers.createAdmin({});
    await helpers.createClient({
      body: { adminWhoRegistered: admin._id },
    });

    return baseRequest({ id: admin._id }, setupData.dev.token).then(
      utils.expectGqlError(err.ADMIN_IN_USE, resolver),
    );
  });

  test('422 Admin in use - by order', async () => {
    const admin = await helpers.createAdmin({});
    await helpers.createOrder({
      body: { seller: admin._id },
    });
  ...
  test('404 Admin not found - already deleted', async () => {
    const id = ents.objects[0]._id;
  ...
  afterAll(async () => {
    await helpers.dropCollections(['Admin', 'User', 'Client', 'Order']);
  });
};
