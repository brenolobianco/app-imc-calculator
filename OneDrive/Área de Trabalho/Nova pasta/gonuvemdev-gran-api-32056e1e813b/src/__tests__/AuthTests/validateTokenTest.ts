...
export default (): void => {
  ...
  test('403 Invalid token - token format', () => {
    return baseRequest({ token: utils.unregisteredToken }).then(
      utils.expectGqlError(err.INVALID_TOKEN, resolver),
    );
  });

  test('403 Invalid token - not token format', () => {
    return baseRequest({ token: 'oi' }).then(
      utils.expectGqlError(err.INVALID_TOKEN, resolver),
    );
  });
  ...
  Object.values(Role).forEach(role => {
    test(`200 Token is valid for ${role}`, () => {
      return baseRequest({ token: setupData[role].token }).then(response => {
