...
export default (): void => {
  ...
  test('404 User not found', () => {
    return baseRequest(fakeBody).then(
      utils.expectGqlError(USER_NOT_FOUND, resolver),
    );
  });
  ...
  test('422 Wrong renew password code - no code generated', async () => {
    ...
    return baseRequest(wrongBody).then(
      utils.expectGqlError(WRONG_RENEW_PASSWORD_CODE, resolver),
    );
  });
  ...
  test('201 Password updated', async () => {
    ...
    return baseRequest(body).then(response => {
