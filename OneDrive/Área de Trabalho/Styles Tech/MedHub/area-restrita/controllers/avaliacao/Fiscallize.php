...
interface FiscallizeInterface {
    public function criarEstudante($name, $email, $password, $enrollmentNumber, $idAvaliacao);
 ...
 *
 * 
 * @copyright      MedHub - http://medhub.app.br/
 * @license        MedHub - http://medhub.app.br/
 * @link           MedHub - http://medhub.app.br/
 * 
 * @since          1.0.0
 * 
 * 
...
class FiscallizeSQL {
    ...
    protected function avaliacoesHospital($idHospital) {
        /**
         * 
         ...
         * @return array
         */
        $provasSQL = "SELECT * FROM estagio LEFT JOIN avaliacoes ON estagio.id_est = avaliacoes.id_est WHERE estagio.id_hosp = :id_hospital";
        $stmt = $this->conexao->prepare($provasSQL);
...
class Fiscallize extends FiscallizeSQL implements FiscallizeInterface {
    ...
    public function criarEstudante($name, $email, $password, $enrollmentNumber, $idTurma = null) {
        /**
         * Criar um estudante
         * 
         ...
         * @param string $enrollmentNumber - identificação de matricula do estudante, pode ser utilizado o ID na plataforma
         * 
         * @return array
         */
        $uri = "/students/";
        ...
        try {
            ...
            $this->inserirEstudanteEmUmaClasse($result["id"], $idTurma);
    ...
    public function rankingHospital($idHospital) {
        /**
         * 
         * 
         ...
         * 
         */
        $provasSQL = "SELECT DISTINCT avaliacoes.avaliacao_id_fiscallize FROM avaliacoes INNER JOIN estagio ON estagio.id_est = avaliacoes.id_est WHERE estagio.hosp_id_est = :id_hospital"; // provas do hospital na Fiscallize
        $stmt = $this->conexao->prepare($provasSQL);
        $stmt->bindParam(":id_hospital", $idHospital);
        $stmt->execute();
        $provas = $stmt->fetchAll();
        $ranking = array("provas" => array());
        foreach($provas as $prova) {
            $id = $prova['avaliacao_id_fiscallize'];
            $resRanking = $this->rankingProva($id);
            $nome = $resRanking[0]->exam;

            $arrayPush = array(
                "id" => $id,
                "ranking" => $resRanking,
                "nome" => $nome,
                // "classe" => $this->classeProva($id),
            );


            array_push($ranking['provas'], $arrayPush);
        }


        $ranking = array_map("objectToArray", $ranking);
        return $ranking;
    }
    ...
    public function rankingPorTurmasEstagioUsuario($idLog) {
        /**
         * Ranking do usuário por turmas no  Estágio dele.
         * 
         * 
