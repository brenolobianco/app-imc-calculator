...
if(isset($_GET['acao'])) {
    ...
    if(isset($_GET['id_aula'])) {
        ...
        if($acao == "get-pre-teste") {
            ...
            if($autorizado) {
                ...
                $select = "SELECT DISTINCT(perguntas.id_pre_teste), perguntas.id_vid_aula, perguntas.pergunta, perguntas.alternativa_a, perguntas.alternativa_b, perguntas.alternativa_c, perguntas.alternativa_d, perguntas.alternativa_e, quiz.resposta as usuario_resposta, quiz.id_usuario FROM treinamento_pre_teste perguntas LEFT JOIN quiz_treinamento_pre_teste_tentivas quiz ON quiz.id_pre_teste = perguntas.id_pre_teste AND quiz.id_usuario = :id_usuario WHERE perguntas.id_vid_aula = :id_vid_aula  GROUP BY perguntas.id_pre_teste";  
                try{
                    $result = $conexao->prepare($select);
    ...
    if($acao == "fiscallize") {
        ...
        $criado = $fiscallize->criarEstudante($nomeLog, $emailLog, $cpf, $idLog, $idTurma);
...
function numero_tentativas_quiz($conexao, $id_usuario, $id_aula) {
    ...
    if($fetch) {
        $num_tentativas = $fetch->num_tentativas;
...
function retornar_video($conexao, $id_aula) {
    ...
    if($contar>0){
        while($mostra = $result->FETCH(PDO::FETCH_OBJ)){
            ...
            $caminho = __DIR__ . "/videos/$id_aula/$arq_vid";
